...
{
    public class GotoDommeOrgasmCommandProcessor : ICommandProcessor
    {
        ...
        public string DeleteCommandFrom(string input) => _lineService.DeleteCommand(input, Keyword.GotoDommeOrgasm);

        public bool IsRelevant(Session session, string line) => line.Contains(Keyword.GotoDommeOrgasm);

        public Result<Session> PerformCommand(Session session, string line)
        {
            var workingSession = session.Clone();
            var result = FindBookmark(workingSession.CurrentScript.Lines.ToList(), "(" + workingSession.Domme.AllowsOrgasms.ToString() + " Allows)")
