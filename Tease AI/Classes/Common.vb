...
#End Region ' TextFiles
    ...
    ''' <summary>
    ''' Scales the image pointed to by <paramref name="imagePath"/> to <paramref name="size"/>
    ''' Should scale and copy to a new memory object.
    ''' </summary>
    ''' <param name="imagePath"></param>
    ''' <param name="size"></param>
    ''' <returns></returns>
    Public Shared Function ScaleImage(imagePath As String, size As Size) As Result(Of Image)
        If Not File.Exists(imagePath) Then
            Return Result.Fail(Of Image)(imagePath + " was not found.")
        End If
        Using originalImage As Bitmap = Image.FromFile(imagePath)
            Dim scaleWidth As Decimal = originalImage.Width / size.Width
            Dim scaleHeight As Decimal = originalImage.Height / size.Height
            Dim scaleFactor As Decimal = Math.Max(scaleWidth, scaleHeight)
            Dim scaledWidth As Integer = originalImage.Width / scaleFactor
            Dim scaledHeight As Integer = originalImage.Width / scaleFactor
            Dim scaledImage As Bitmap = New Bitmap(scaledWidth, scaledHeight)
            scaledImage.SetResolution(originalImage.HorizontalResolution, originalImage.VerticalResolution)
            Using graphic As Graphics = Graphics.FromImage(scaledImage)
                graphic.InterpolationMode = Drawing2D.InterpolationMode.HighQualityBicubic
                graphic.DrawImage(originalImage, New Rectangle(0, 0, scaledWidth, scaledHeight), 0, 0, originalImage.Width, originalImage.Height, GraphicsUnit.Pixel)
            End Using
            Return Result.Ok(CType(scaledImage, Image))
        End Using
    End Function
