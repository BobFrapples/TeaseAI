...
Public Class CustomSlideshow
    Inherits Dictionary(Of String, ImageGenre)
    Implements ISerializable, IDeserializationCallback

    Sub New()
    End Sub

    Sub New(ByVal copy As Dictionary(Of String, ImageGenre))
        MyBase.New(copy)
    End Sub
    ...
    Public Sub AddRange(ByVal List As List(Of String), ByVal genre As ImageGenre)
    ...
    <NonSerialized> Dim lastgenre As ImageGenre
    ''' <summary>
    ...
    Public Function GetRandom(ByVal preferOffline As Boolean) As String
        Try
            ...
            Dim Genrelist As List(Of ImageGenre) = Me.Values.Distinct.ToList

            ' Set the maximum retries to get a new new genre
            retryCounter = CInt(Genrelist.Count / 4)
PickNewGenre:
            ' Pick a random genre.
            Dim rndGenre As ImageGenre = Genrelist(RandomInteger(0, Genrelist.Count))
...
pickNextImage:
    ...
    Sub RemoveGenre(ByVal genre As ImageGenre)
