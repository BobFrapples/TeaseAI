...
#Region "---------------------------------------------------- Create URL Files --------------------------------------------------"
        ...
        Private Function BlogToUrlFile(ByVal Optional ___ImageBlogUrl As String = "") As CreateUrlFileResult
            ...
            '===============================================================================
            Dim ___ModifiedUrl As String = GetLocalUrl(___ImageBlogUrl)
...
Scrape:
                ...
                '===============================================================================
                If ___ImageBlogUrl.Last() = "/" Then
                    ___ImageBlogUrl = ___ImageBlogUrl.Remove(___ImageBlogUrl.Length - 1)
                End If
                ...
                For Each ___PhotoNode As XmlNode In ___doc.DocumentElement.SelectNodes("//photo-url")
                    Try
                        ...
                        If CInt(___PhotoNode.Attributes.ItemOf("max-width").InnerText) = 1280 Then
                            ...
                            ElseIf Me._Work = URL_File_Tasks.RebuildURLFiles Then
                                '########################## URL-Rebuild - Unknown URL ############################
                                ' If Rebuilding URL-File skip Urls not previous known.
                                GoTo NextImage
                            ElseIf Me._Work = URL_File_Tasks.RefreshURLFiles AndAlso ___BlogListOld.Contains(___PhotoNode.InnerXml) Then
                                '############################# Refresh - Known URL ###############################
                                ' If refreshing a URL-File and there is a known URL then stop scraping.
                                GoTo ExitScrape
                            ElseIf ___BlogListOld.Contains(___PhotoNode.InnerXml) Then
                                '############################## Create - Known URL ###############################
                                ___BlogListNew.Add(___PhotoNode.InnerXml)           ' Add to new list
                                ___ImageCountTotal += 1                             ' Increment Image Counter.
                                GoTo NextImage                                      ' No Saving or Reviewing    
...
#Region "-------------------------------------------------- Misc. Functions ---------------------------------------------------"
        ...
        ''' <summary>
        ''' This strips "http://", "https://" and '/' from <paramref name="blogUrl"/> and returns the result
        ''' </summary>
        ''' <param name="blogUrl"></param>
        ''' <returns></returns>
        Private Function GetLocalUrl(blogUrl As String) As String
            Return blogUrl.Replace("http://", "").Replace("https://", "").Replace("/", "")
        End Function
