...
#Region "------------------------------------------- Data -----------------------------------------------"
    ...
    ''' <summary>
    ''' I think this is how many seconds the tease will last
    ''' </summary>
    ''' <returns></returns>
    Public Property TeaseTick As Integer
    Public Property Responding As Boolean
    ''' <summary>
    ''' This appears to be used to track line number of scripts
    ''' </summary>
    ''' <returns></returns>
    <Category("Taunts")> Public Property StrokeTauntVal As Integer
        Get
            Return myStrokeTauntVal
        End Get
        Set(value As Integer)
            If value > myScriptLineCount Then
                Throw New Exception("Past end of script")
            End If
    ...
    <Category("Script")> Public Property FileText As String
        ...
        End Get
        Set(value As String)
            myScriptLineCount = File.ReadAllLines(value).Count()
            myFileText = value
        End Set
    End Property
    ...
    ''' <summary>
    ''' I think this means the domme isn't responding
    ''' </summary>
    ''' <returns></returns>
    ...
    ''' <summary>
    ''' Is the Domme away from the keyboard
    ''' </summary>
    ''' <returns></returns>
    ...
    ''' <summary>
    ''' This appears to only be used to determine if we are in the first round of stroking
    ''' </summary>
    ''' <returns></returns>
...
#Region "------------------------------------- Constructors----------------------------------------------"
    ...
    Sub New(ByVal ActivationForm As MainWindow)
        InitializeComponent()
...
#Region "----------------------------------- Get/Set Form1 Data -----------------------------------------"
    ...
    Sub FetchFormData(ByVal serializeForm As MainWindow)
        ...
        AndAlso serializeForm.InvokeRequired Then
            ...
            Dim Act As Action(Of MainWindow) = Sub(s1) FetchFormData(s1)
    ...
    Friend Sub Activate(ByVal activateForm As MainWindow)
        ' Check if InvokeIsRequired
        If activateForm IsNot Nothing _
        AndAlso activateForm.InvokeRequired Then
            ' Calling from another Thread -> Invoke on controls UI-Thread
            Dim Act As Action(Of MainWindow) = Sub(s1) Activate(s1)
...
#End Region ' Get/Set Data
    ...
    ''' and applies a new <see cref="SessionState"/>-instance to <see cref="MainWindow.ssh"/>.
    ''' <para></para>
